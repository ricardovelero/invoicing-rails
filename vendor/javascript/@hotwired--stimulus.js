...
function parseActionDescriptorString(t) {
  const r = t.trim();
  const s = r.match(e) || [];
  return {
    eventTarget: parseEventTarget(s[4]),
    eventName: s[2],
    eventOptions: s[9] ? parseEventOptions(s[9]) : {},
    identifier: s[5],
    methodName: s[7],
  };
}
...
class Action {
  ...
  static forToken(e) {
    return new this(e.element, e.index, parseActionDescriptorString(e.content));
  }
  toString() {
  ...
  get params() {
    const e = {};
    const t = new RegExp(`^data-${this.identifier}-(.+)-param$`);
...
class Binding {
  ...
  handleEvent(e) {
    if (this.willBeInvokedByEvent(e) && this.shouldBeInvokedPerSelf(e)) {
  ...
  invokeWithEvent(e) {
    ...
    try {
      ...
      const n = Object.assign(e, { params: s });
  ...
  shouldBeInvokedPerSelf(e) {
    return (
      true !== this.action.eventOptions.self || this.action.element === e.target
    );
  }
...
class ElementObserver {
  ...
  processAttributeChange(e, t) {
    const r = e;
    this.elements.has(r)
      ? this.delegate.elementAttributeChanged && this.matchElement(r)
        ? this.delegate.elementAttributeChanged(r, t)
        : this.removeElement(r)
      : this.matchElement(r) && this.addElement(r);
  }
  ...
  invokeChangedCallback(e, t, r) {
    ...
    if ("function" == typeof n) {
      ...
      try {
        ...
        throw new TypeError(
          `Stimulus Value "${this.context.identifier}.${s.name}" - ${e.message}`
        );
      }
    }
  }
...
function shadow(e, t) {
  const r = s(e);
  const n = getShadowProperties(e.prototype, t);
...
class ScopeObserver {
  ...
  parseValueForToken(e) {
    ...
    const s = this.fetchScopesByIdentifierForElement(t);
    let n = s.get(r);
    if (!n) {
      n = this.delegate.createScopeForElementAndIdentifier(t, r);
      s.set(r, n);
    }
    return n;
  }
...
class Application {
  constructor(e = document.documentElement, t = n) {
    this.logger = console;
    this.debug = false;
...
function parseValueTypeObject(e) {
  const t = parseValueTypeConstant(e.typeObject.type);
  if (!t) return;
  const r = parseValueTypeDefault(e.typeObject.default);
  if (t !== r) {
    const s = e.controller ? `${e.controller}.${e.token}` : e.token;
    throw new Error(
      `The specified default value for the Stimulus Value "${s}" must match the defined type "${t}". The provided default value of "${e.typeObject.default}" is of type "${r}".`
    );
  }
  return t;
}
function parseValueTypeDefinition(e) {
  const t = parseValueTypeObject({
    controller: e.controller,
    token: e.token,
    typeObject: e.typeDefinition,
  });
  const r = parseValueTypeDefault(e.typeDefinition);
  const s = parseValueTypeConstant(e.typeDefinition);
  const n = t || r || s;
  if (n) return n;
  const i = e.controller ? `${e.controller}.${e.typeDefinition}` : e.token;
...
function valueDescriptorForTokenAndTypeDefinition(e) {
  const t = `${dasherize(e.token)}-value`;
  const r = parseValueTypeDefinition(e);
  return {
    type: r,
    key: t,
    name: camelize(t),
    get defaultValue() {
      return defaultValueForDefinition(e.typeDefinition);
    },
    get hasCustomDefaultValue() {
      return void 0 !== parseValueTypeDefault(e.typeDefinition);
    },
    reader: o[r],
    writer: c[r] || c.default,
  };
}
const i = {
  get array() {
...
const c = { default: writeString, array: writeJSON, object: writeJSON };
