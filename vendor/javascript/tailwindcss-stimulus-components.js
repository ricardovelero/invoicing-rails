...
class i extends t {
    ...
    setTimeout(
      (() => {
        this.menuTarget.classList.remove(this.toggleClass),
          ...
          this._activeClassList[0].forEach((t) => {
            this.activeTarget.classList.add(t);
          }),
          this._invisibleClassList[0].forEach((t) =>
            this.menuTarget.classList.remove(t)
          ),
          this._visibleClassList[0].forEach((t) => {
            this.menuTarget.classList.add(t);
          }),
          setTimeout(
            (() => {
              this._enteringClassList[0].forEach((t) =>
                this.menuTarget.classList.remove(t)
              );
            }).bind(this),
            this.enterTimeout[0]
    ...
    setTimeout(
      (() => {
        this.element.setAttribute("aria-expanded", "false"),
          ...
          this._activeClassList[0].forEach((t) =>
            this.activeTarget.classList.remove(t)
          ),
          this._leavingClassList[0].forEach((t) =>
            this.menuTarget.classList.add(t)
          ),
          setTimeout(
            (() => {
  ...
  hide(t) {
    !1 === this.element.contains(t.target) &&
      this.openValue &&
      (this.openValue = !1);
  }
  get activeTarget() {
  ...
  get _activeClassList() {
    return this.activeClass
      ? this.activeClass.split(",").map((t) => t.split(" "))
      : [[], []];
  }
  get _visibleClassList() {
    return this.visibleClass
      ? this.visibleClass.split(",").map((t) => t.split(" "))
      : [[], []];
  }
  get _invisibleClassList() {
    return this.invisibleClass
      ? this.invisibleClass.split(",").map((t) => t.split(" "))
      : [[], []];
  }
  get _enteringClassList() {
    return this.enteringClass
      ? this.enteringClass.split(",").map((t) => t.split(" "))
      : [[], []];
  }
  get _leavingClassList() {
    return this.leavingClass
      ? this.leavingClass.split(",").map((t) => t.split(" "))
      : [[], []];
  }
  get enterTimeout() {
    return (this.data.get("enterTimeout") || "0,0")
      .split(",")
      .map((t) => parseInt(t));
  }
  get leaveTimeout() {
    return (this.data.get("leaveTimeout") || "0,0")
      .split(",")
      .map((t) => parseInt(t));
  }
}
...
class a extends t {
  connect() {
    (this.toggleClass = this.data.get("class") || "hidden"),
      ...
      (this.allowBackgroundClose =
        "true" === (this.data.get("allowBackgroundClose") || "true")),
      (this.preventDefaultActionOpening =
        "true" === (this.data.get("preventDefaultActionOpening") || "true")),
  ...
  open(t) {
    this.preventDefaultActionOpening && t.preventDefault(),
      t.target.blur && t.target.blur(),
      this.lockScroll(),
      this.containerTarget.classList.remove(this.toggleClass),
      this.data.get("disable-backdrop") ||
        (document.body.insertAdjacentHTML("beforeend", this.backgroundHtml),
        (this.background = document.querySelector(`#${this.backgroundId}`)));
  }
  close(t) {
    t && this.preventDefaultActionClosing && t.preventDefault(),
      this.unlockScroll(),
  ...
  unlockScroll() {
    (document.body.style.paddingRight = null),
      ...
      this.restoreScrollValue && this.restoreScrollPosition(),
...
class l extends t {
  connect() {
    (this.activeTabClasses = (this.data.get("activeTab") || "active").split(
      ...
      this.anchor &&
        (this.index = this.tabTargets.findIndex((t) => t.id === this.anchor)),
      this.showTab();
  }
  change(t) {
    t.preventDefault(),
      (this.index = t.currentTarget.dataset.index
        ...
        ? this.tabTargets.findIndex((d) => d.id == t.currentTarget.dataset.id)
        : this.tabTargets.indexOf(t.currentTarget)),
      window.dispatchEvent(new CustomEvent("tsc:tab-change"));
  }
  showTab() {
    this.tabTargets.forEach((t, d) => {
      const c = this.panelTargets[d];
      d === this.index
        ? (c.classList.remove("hidden"),
          t.classList.remove(...this.inactiveTabClasses),
          t.classList.add(...this.activeTabClasses),
          t.id && (location.hash = t.id))
        : (c.classList.add("hidden"),
          t.classList.remove(...this.activeTabClasses),
          t.classList.add(...this.inactiveTabClasses));
    });
  }
  get index() {
...
class r extends i {
  _show() {
    this.overlayTarget.classList.remove(this.toggleClass),
      super._show(
        (() => {
          this._activeClassList[1].forEach((t) =>
            this.overlayTarget.classList.add(t)
          ),
            this._invisibleClassList[1].forEach((t) =>
              this.overlayTarget.classList.remove(t)
            ),
            this._visibleClassList[1].forEach((t) =>
              this.overlayTarget.classList.add(t)
            ),
            setTimeout(
              (() => {
                this._enteringClassList[1].forEach((t) =>
                  this.overlayTarget.classList.remove(t)
                );
              }).bind(this),
              this.enterTimeout[1]
  ...
  _hide() {
    this._leavingClassList[1].forEach((t) =>
      this.overlayTarget.classList.add(t)
    ),
      super._hide(
        (() => {
...
class h extends t {
  ...
  set preview(t) {
    ...
    const d = this._getContrastYIQ(t);
    "color" === this.styleProperty
      ? (this.previewTarget.style.backgroundColor = d)
      : (this.previewTarget.style.color = d);
  }
  get color() {
